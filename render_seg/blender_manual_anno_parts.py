...
# -*- coding: utf-8 -*-
'''
blender_manual_anno_parts.py
brief:
    render projections of a 3D model and its annotated parts from viewpoints specified by an input parameter file
usage:
	blender blank.blend --background --python blender_manual_anno_parts.py -- <anno_savedir> <model_id> <model_dir> <anno_dir> <par_file>
...
def main():
    ...
    part_list_file = os.path.join(anno_dir, 'part_list.txt')
    assert os.path.exists(part_list_file)
    ...
    scene.render.image_settings.file_format = 'OPEN_EXR' # set the format.
    ...
    for part_idx in range(len(part_list)):
        shape_file = os.path.join(anno_dir, model_id,'{}.obj'.format(part_list[part_idx]))
        if not os.path.exists(shape_file):
            print('no existing obj file: ' + shape_file)
            continue
...
def render_obj(view_params, shape_file, syn_image_file_template):
    ...
    for param in view_params:
        ...
        syn_image_file = syn_image_file_template.format(round(azimuth_deg), round(elevation_deg), 
                                                        round(theta_deg), round(rho*100))
